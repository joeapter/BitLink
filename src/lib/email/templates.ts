...
// ── Admin-triggered password reset — the same recovery link the customer
// would get from "Forgot password" themselves, just generated by an admin
// on their behalf (support call, WhatsApp message, "I'm locked out") and
// sent from the line's admin page instead of self-serve. Deliberately says
// this reached them because BitLink support sent it — someone who did not
// ask for a reset and gets an unmarked "reset your password" email has
// reason to worry their account was compromised.

export function buildAdminResetLinkEmail(params: {
  fullName: string;
  resetUrl: string;
}): string {
  ...
  return layout(`
    ${h1(`Reset your BitLink password, ${firstName}`)}
    ${p("BitLink support generated this link for you so you can set a new password.")}
    <div style="text-align:center;margin:28px 0;">
      ${btn("Set a new password", params.resetUrl)}
    </div>
    ${p("This link is single-use and expires soon, so use it shortly after you get this email. If it's stopped working by the time you click it, just ask us for a new one.")}
    ${p("Didn't ask for this? Nothing has changed on your account yet — reply to this email and let us know.")}
  `);
}

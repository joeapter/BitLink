...
interface Props {
  ...
  // Set when the buckets are derived from CDR records rather than a live
  // provider balance (see lib/cdr/usage.ts).
  note?: string;
  expiryLabel?: string;
}
...
export function LineBalanceCard({ balances, note, expiryLabel = "Expires" }: Props) {
  if (!balances.length) {
    return (
    ...
    <section className="rounded-[2rem] border border-ink/10 bg-white p-6 shadow-soft">
      ...
      <div className="mt-5 grid gap-4">
        {balances.map((b) => {
          ...
          const isVoiceBucket = isVoice(b);
          const fmtValue = (v: number) =>
            isDataBucket ? fmt(v) : isVoiceBucket ? fmtMinutes(v) : String(Math.round(v));
          const bar =
            usedPct > 90
              ? "bg-red-500"
              : usedPct > 70
            ...
            <div key={b.id} className="rounded-xl border border-ink/8 bg-slate-50 p-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-muted-slate">
                  {expiryLabel} {b.expirationDate.toLocaleDateString()}
                </p>
              </div>
              ...
              <div className="mt-2 flex items-end justify-between text-xs text-muted-slate">
                <span>
                  Used:{" "}
                  <span className="font-semibold text-ink">{fmtValue(used)}</span>
                </span>
                <span>
                  Remaining:{" "}
                  <span className="font-semibold text-ink">{fmtValue(b.value)}</span>
                </span>
              </div>
